...
export type Part = {
  ...
  materials: string;
  bodyMaterial?: MaterialDescription;
  roofMaterial?: MaterialDescription;
  wallHeight?: number;
...
export function validate(raw: unknown): Plan {
  ...
  const material = (m: unknown) =>
    m === undefined ||
    (!!m &&
      typeof m === "object" &&
      str((m as MaterialDescription).name) &&
      str((m as MaterialDescription).description));
  ...
  for (const p of d.parts) {
    if (
      ...
      !str(p.materials) ||
      !material(p.bodyMaterial) ||
      !material(p.roofMaterial) ||
